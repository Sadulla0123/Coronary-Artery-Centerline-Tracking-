    ...
    for v in range(4):
        ...
        for j in range(start_ind, end_ind + 1):
            if j % gap_size == 0:
                ...
                if (org_x_pixel != last_center_x_pixel or
                    org_y_pixel != last_center_y_pixel or
                    org_z_pixel != last_center_z_pixel):
                    ...
                    for k in range(10):
                        ...
                        while True:
                            if ((center_x_pixel != org_x_pixel or
                                 center_y_pixel != org_y_pixel or
                                 center_z_pixel != org_z_pixel) and
                        ...
                        for s_ind in range(len(sx)):
                            shell_arr[s_ind] = [sx[s_ind], sy[s_ind], sz[s_ind]]
                        ...
                        if pre_ind != -1 and next_ind != -1:
                            ...
                            new_src_arr = np.zeros((cut_size * 2 + 1,
                                                    cut_size * 2 + 1,
                                                    cut_size * 2 + 1), dtype=re_spacing_img.dtype)

                            for ind in range(left_z, right_z + 1):
                                if ind < 0 or ind >= re_spacing_img.shape[0]:
                                    continue  # skip out-of-bounds z

                                src_temp = re_spacing_img[ind].copy()
                                patch = src_temp[max(0, left_y):min(src_temp.shape[0], right_y + 1),
                                                 max(0, left_x):min(src_temp.shape[1], right_x + 1)]

                                # Pad to (19,19)
                                padded_patch = np.zeros((cut_size * 2 + 1, cut_size * 2 + 1),
                                                        dtype=src_temp.dtype)
                                y_size, x_size = patch.shape
                                padded_patch[:y_size, :x_size] = patch
                                new_src_arr[ind - left_z] = padded_patch
                            ...
                            if np.random.uniform() <= rotate_prob:
                                ...
                                new_src_arr, new_pre_cood, new_next_cood = rotate_augmentation(
                                    new_src_arr, pre_ind, next_ind, curr_c, center,
                                ...
                                pre_x, pre_y, pre_z = center[pre_ind]
                                next_x, next_y, next_z = center[next_ind]
                                curr_c = [center_x_new, center_y_new, center_z_new]
                                pre_sim = find_closer_point_angle(shell_arr, [pre_x, pre_y, pre_z], curr_c)
                                next_sim = find_closer_point_angle(shell_arr, [next_x, next_y, next_z], curr_c)
                                pre_ind_list.append(pre_sim)
                                next_ind_list.append(next_sim)
                                radials_list.append(new_radial)
...
def create_patch_images(max_points, path_name, spacing_path, gap_size):
    for i in range(8):
        ...
        csv_name = "./patch_data/centerline_patch/offset/" + 'point_' + str(max_points) + '_gp_' + str(gap_size) + '/' + 'd' + str(i) + "_patch_info_%d.csv" % max_points
