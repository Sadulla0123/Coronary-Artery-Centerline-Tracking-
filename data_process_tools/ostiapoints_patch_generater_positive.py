...
def creat_data(path_name, spacing_path, gap_size, save_num):
    ...
    # Build image path
    image_pre_fix = os.path.join(path_name + '0' + str(i), 'image' + '0' + str(i))
            ...
            if k == 0:
                ...
                if ((right_z + 1) < len(re_spacing_img) and left_z >= 0 and
                    ...
                    folder_path = os.path.join(
                        '.', 'patch_data', 'ostia_patch', 'postive',
                    ...
                    record_name = os.path.join(
                        'ostia_patch', 'postive',
                        f'gp_{gap_size}', f'd{i}',
                        f'd_{i}_x_{center_x_pixel}_y_{center_y_pixel}_z_{center_z_pixel}.nii.gz'
                    )
                    print(record_name)

                    org_name = os.path.join('.', 'patch_data', record_name)
...
def create_patch_images(path_name, spacing_path, gap_size):
    for i in range(8):
        ...
        dataframe = pd.DataFrame({'patch_name': patch_name, 'proximity': proximity_list})
        print(dataframe.head())

        csv_name = os.path.join(
            '.', 'patch_data', 'ostia_patch', 'postive',
            f'gp_{gap_size}', f'd{i}_patch_info.csv'
        )
...
if __name__ == "__main__":
    path_name = os.path.join('train_data', 'dataset')
